...
따라서 로컬 `DB` 혹은 서버와 통신하여 데이터를 `Domain` 모델의 데이터로 `mapping` 시켜주는 역할을 하는 `Mapper` 을 사용하여 `Presentation` 레이어가 `Data` 레이어에 영향을 받지 않도록 할 수 있다.
<br/>
<br/>
...
### Presentation 레이어
`UI` 에 관련된 기능을 담당하고 있으며 `Activity`, `Fragment`, `ViewModel` 등을 포함하고 있으며 `Domain` 레이어에 대한 의존성을 가지고 있다.