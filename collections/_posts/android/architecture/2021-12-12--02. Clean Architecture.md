...
안드로이드에서 클릭 아키텍처는 `Data`, `Domain`, `Presentation(UI)` 레이어로 구성되며 각 레이어마다 역할과 책임을 명확히 가지고 있으며, 각 레이어의 모듈들은 단방향의 참조를 의미한다.
<br/>
...
***
"`A` 는 상품을 구매하기 위해 이커머스 앱을 실행시키고 필요한 상품을 클릭하였다." 라고 하는 단순한 동작 하나라도 앱 내에서는 각 레이어들의 단방향 데이터 흐름(`UpStream` + `DownStream`)으로 통신이 시작된다.
<br/>
...
`Presentation(UI)` 레이아어서 가장 중요한 역할을 하는 것은 `ViewModel` 로 3가지 책임을 가지고 있다.
- `UpStream` 관점에서 비즈니스 로직의 출발 지점으로 사용자에게 `UI` 이벤트를 받아야하고 이를 `Domain` 혹은 `Data` 레이어에 이벤트를 전달하며 비즈니스 로직의 시작을 트리거하는 책임이 있다.
...
class GetSearchUseCase @Inject constructor(
    ...
    operator fun invoke(query: String): Flow<List<SearchModel>> = flow {
        coroutineScope {
            ...
            val images = response.await()
            val videos = entity.await()
...
- 가공한 데이터를 `Repository` 모듈에 캐싱하는 책임이 있다.
<br/>
<br/>

`DataSource` 는 네트워크 통신을 통한 `API` 서버 혹은 로컬 `DB` 로부터 실제 데이터를 가져오는 역할로 2가지 책임을 가지고 있다.
